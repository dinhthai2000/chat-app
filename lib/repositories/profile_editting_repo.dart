...
class ProfileEdittingRepository {
  ...
  Future<ChatUserModel?> getCurrentUser() async {
    try {
      ...
      // var userImgP = _auth.currentUser?.phoneNumber;
      // var userImgE = _auth.currentUser?.email;
      // var userImge = await _store.collection('usersImg').doc(userImgE).get();
      // var userImgp = await _store.collection('usersImg').doc(userImgP).get();
